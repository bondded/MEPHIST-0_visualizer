...
class TokamakApp:
    ...
    def create_widgets(self):
        ...
        self.ax_ip.set_xlabel('Time [ms]')
        self.ax_halpha.set_ylabel('H-alpha [a.u.]')
        self.ax_halpha.set_xlabel('Time [ms]')
    ...
    def on_mouse_move(self, event):
        ...
        # only act if cursor is in a time axis (not in spectrum)
        if event.inaxes == self.ax_avantes:
            return
    ...
    def on_right_click(self, event):
        ...
        if event.inaxes == self.ax_avantes:  # don't copy spectrum data
            return
...
if __name__ == "__main__":
    ...
    root = tk.Tk()
    style = ttk.Style()
    style.theme_use('clam') # Un tema más moderno y limpio en Linux
